...
public class Round {
    ...
    /**
     * AI method to have the computer player whose turn it is choose to play 
     * the highest valued attribute on their card.
     * 
     ...
     * all cards in play go to the communal pile
     * 
     * @param cardsInPlay, Card[]
     * @param winner, Player
     */
    ...
    /**
     * Returns whether the user has won the game (has all of the cards in
     * the pack).
     * 
    ...
    /**
     * Generates and returns a formatted String indicating the Cards each 
     * Player has left.
     * 
    ...
    private String getRoundTrumpString() {
        ...
        String s = String.format("Previous round attribute: %s%n", 
    ...
    private void printTrumpInfo() {
        ...
        for (int i = 0; i < this.players.length; i++) {
            ...
            if (p.getHandSize() > 0) {
                ...
                System.out.println(p.getName()+" trump score:"
                        + " "+c.getAttriValAtIndex(this.trumpIndex)+"\n");
            }
        }
        System.out.println("");        
