...
public class TopTrumpsGUI extends JFrame implements ActionListener {
    ...
    public TopTrumpsGUI() throws HeadlessException {
        ...
        // Combo box
        String[] numPlayerOptions = {"2 Players","3 Players",
        ...
        this.outputScrollPane = new JScrollPane (outputTextArea, 
        ...
        buttonPan.add(playAttr1Button);
        buttonPan.add(playAttr2Button);
        buttonPan.add(playAttr3Button);
        ...
        // Creates input panel
        GridLayout inputGrid = new GridLayout(2,1);
    ...
    /**
     * Attempts to read deck information Strings from a text file, line
     * by line and write them to an array; if unsuccessful, informs user 
     * with error message and performs a system exit.
     * 
     ...
     * 
     * @param deckLineStrings, card attribute information
    ...
    public Card[] generateCardsArray(String[] deckLineStrings) {
        ...
        for (int i = 1; i < deckLineStrings.length; i++) {
            ...
            Card newCard = new Card(cardName, attri1, attri2, attri3, 
                    attri4, attri5);
    ...
    /**
     * Creates new Round, plays round, updates display text
     * information and checks if game is over.
     * 
     * @param trumpIndex, the index of the trump for a given round: 1-5 if
    ...
    /**
     * Checks to see if the game is over; updates game information and
     * finishes game if it is.
     */
    private void checkIfGameOver() {
        
        if (this.currentRound.boolUserWonGame() || 
                this.currentRound.boolUserLostGame()) {
    ...
    /**
     * Displays a pop-up informing the user that the game is over, telling 
     * them the identity of the winner and asking them whether they wish 
     * to save the game data to database.
     */
    ...
    /**
     * Sets whether the winner of the current game was a human or the 
     * computer.
     */
    ...
    /**
     * Updates the current Game information to indicate whether the result of
     * a game was a computer win, a human win or a draw and increments the 
    ...
    /**
     * Updates the Game's deciding Player, in the case of a Round not ending
     * in a draw.
     */
    ...
    /**
     * Instantiates Status Report object which queries a database to get data
     * on previous games played, formats that data and displays it in a new 
     * window.
     */
    ...
    private String generateCurrentCardString() {
        ...
        String CardDescription = String.format(this.USER_NAME+" - current card:"
                + " %n Description: %s%n", UserCurrentCard.getNameVal());
        String CardAttri1 = String.format(
                "%s: %s     ", currentDeck.getAttriNameAtIndex(1),
                UserCurrentCard.getAttriValAtIndex(1));
        String CardAttri2 = String.format(
                "%s: %s     ", currentDeck.getAttriNameAtIndex(2),
                UserCurrentCard.getAttriValAtIndex(2));
        String CardAttri3 = String.format(
                "%s: %s     ", currentDeck.getAttriNameAtIndex(3),
                UserCurrentCard.getAttriValAtIndex(3));  
        String CardAttri4 = String.format(
                "%s: %s     ", currentDeck.getAttriNameAtIndex(4),
                UserCurrentCard.getAttriValAtIndex(4));      
        String CardAttri5 = String.format(
                "%s: %s     %n%n", currentDeck.getAttriNameAtIndex(5),
    ...
    /**
     * Sets gameplay buttons for  around where a computer play chooses the 
     * trump.
     */
    ...
    /**
     * Sets gameplay buttons for  around where the user chooses the 
     * trump.
     */    
    ...
    private void printDeckAttributeNames() {
        ...
        String attributeNameString = String.format("%20.20s %15.15s %15.15s "
                + "%15.15s %15.15s %15.15s", "", attribute1Name, 
                attribute2Name, attribute3Name, attribute4Name, 
                attribute5Name);
    ...
    private void printCard(Card c) {
        ...
        String attValString = String.format("%20.20s %15.15s %15.15s "
                + "%15.15s %15.15s %15.15s", nameValue, 
    ...
    /**
     * Selects a randomly selected Player as the deciding Player to choose
     * the trump for the first round.
     */
    ...
    private void generatePlayers() {
        ...
        String[] CompPlayerNames = {"WatsonBot", "Amiga64Bot", 
            "BabbageBot", "TuringBot"};
